...
def _parse_date(raw: str) -> Optional[str]:
    ...
    date_patterns = [
        ...
        r"\d{2}-\d{2}-\d{4}",  # DD-MM-YYYY
        r"\d{2}\s+\w{3}\s+\d{4}",  # DD MMM YYYY (e.g. 01 Apr 2025)
        r"\d{1,2}[-\/.][A-Za-z]{3}[-\/.]\ d{2,4}",  # DD-MMM-YYYY (e.g. 01-Apr-2025)
        r"\d{1,2}[-\/.][A-Za-z]{3}\s+\d{4}",  # DD-MMM YYYY or DD/MMM YYYY
...
def _is_line_start_of_tx(line: str) -> bool:
    """Check if line starts with a date (indicates start of a transaction)."""
    return _parse_date(line[:25]) is not None


def _parse_transactions_from_text(text: str, bank_name: str) -> list[ParsedTransaction]:
    """
    Parse transactions from extracted text.
    Enhanced to handle SBI-like concatenated rows by globally splitting by dates first.
    """
    transactions = []
    ...
    # Step 2: heuristic to detect if pdfminer returned a single long concatenated line
    # If we have only 1 raw line but it contains many date tokens, split by dates
    ...
    if len(raw_lines) == 1 and len(all_date_matches) > 3:
        # split full text into parts starting at each date match
        lines = []
        ...
        # Merge wrapped lines: if a line does not start with a date, it's likely a continuation
        lines = []
        current = ""
        for l in raw_lines:
            if not l:
    ...
    # Step 3: for each merged line, split again by date tokens (in case one line has multiple dates)
    parsed_set = set()
...
def extract_transactions_from_pdf(pdf_bytes: bytes) -> list[ParsedTransaction]:
    """
    Main entrypoint: extract transaction list from PDF.
    Prioritizes pdfminer, falls back to pdf2image + OCR if needed.
    """
    transactions = []
    extracted_text = ""
    
    # Try 1: pdfminer.six text extraction
    try:
    ...
    # Try 2: fallback to PyMuPDF (fitz)
    ...
    # If still empty, try image-based OCR
